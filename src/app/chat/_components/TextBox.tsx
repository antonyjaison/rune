  ...
  useEffect(() => {
    switch (suggestion) {
      case "generate_notes":
        setSelected(
          <button
            style={{ boxShadow: "0px 0px 4px 0px #F8BD00" }}
            className=" text-[#F8BD00] font-inter text-sm font-medium bg-white rounded-md px-3 py-2"
          >
            Generate notes
          </button>
        );
      ...
      case "create_flashcards":
        setSelected(
          <button
            style={{ boxShadow: "0px 0px 4px 0px #3A78F2" }}
            className=" text-[#3A78F2] font-inter text-sm font-medium bg-white rounded-md px-3 py-2"
          >
            Create flashcards
          </button>
        );
      ...
      case "explain":
        setSelected(
          <button
            style={{ boxShadow: "0px 0px 4px 0px #FF7F50" }}
            className=" text-[#FF7F50] font-inter text-sm font-medium bg-white rounded-md px-3 py-2"
          >
  ...
  useEffect(() => {
    switch (option) {
      case "invite_friends":
        setSelected(
          <button
            style={{ boxShadow: "0px 0px 4px 0px #334155" }}
            className=" text-[#334155] font-inter text-sm font-medium bg-white rounded-md px-3 py-2"
          >
            Invite friends
          </button>
        );
    ...
    <div className="w-full flex justify-between h-fit items-center gap-3 px-5">
      ...
      <div className="flex-1">
            ...
            <Command className="rounded-b-none">
              <CommandList>
                <CommandGroup className="font-inter text-[#334155] text-xl font-medium">
                  <p className="font-inter text-[#334155] text-base font-semibold pt-1 pl-1 pb-1">
                    Options
                  </p>
                  <CommandItem className="div-hover-scale cursor-pointer">
                    <div
                      onClick={() => handleOptionChange("invite_friends")}
                  ...
                  <CommandItem className="div-hover-scale cursor-pointer">
                    <div
                      onClick={() => handleOptionChange("chat_with_friends")}
                      className="flex h-fit items-center gap-3"
                    >
                ...
                <CommandGroup className="font-inter text-[#334155] text-xl font-medium">
                  ...
                  {suggestions.map((suggestion: SuggestionType) => (
                    <CommandItem
                      key={suggestion.id}
                      className="div-hover-scale cursor-pointer"
                    >
                      <div
                        onClick={() =>
                          handleSuggestionChange(suggestion.option)
                        }
        ...
        {selected && (
          <div className="w-full px-[1px]">
            <Command className="rounded-b-none">
              <CommandList>
                <CommandGroup className="font-inter text-[#334155] text-xl font-medium">
                  <CommandItem disabled>{selected}</CommandItem>
                </CommandGroup>
              </CommandList>
            </Command>
          </div>
        ...
        <Input
          ...
          multiline={true}
          className={cn(
            "p-3 border border-[#EBEDF0] rounded-md font-inter font-normal text-[#0F172A]",
